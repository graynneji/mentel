...
interface ConversionParams {
   ...
   * When provided, the event fires at most once per key per browser. */
  dedupeKey?: string;
  /** Passed as Meta's `eventID` so a server-side CAPI call for the same
   * conversion (see lib/fbConversion.ts) can be deduped against this
   * client-side Pixel fire, per Meta's documented eventID matching. Falls
   * back to dedupeKey if not set, since they're usually the same value in
   * practice (a payment/lead reference). */
  eventId?: string;
}
...
export function fireConversion(event: ConversionEvent, params: ConversionParams = {}) {
  ...
  window.fbq?.("track", event, {
    ...
    currency: params.currency ?? "NGN",
    content_name: params.contentName,
  }, params.eventId || params.dedupeKey ? { eventID: params.eventId ?? params.dedupeKey } : undefined);
