...
// //
// // The branded PDF report, built with @react-pdf/renderer so it can be
// // generated on the server (no headless browser needed). Brand tokens
// // (Cormorant Garamond / DM Sans, deep emerald #0E5C3D) follow the print
// // standard used across all other Mentel documents, distinct from the
// // web app's slightly brighter #2d7a5a used for buttons/UI.

// import React from "react";
// import { Document, Page, Text, View, StyleSheet, Font, Svg, Rect, Path } from "@react-pdf/renderer";
// import { AssessmentResult, Band, bandCopy } from "./scoring";
...
// // ── Fonts ─────────────────────────────────────────────────────────────────
// // Registered from Google Fonts static files at render time (see
// // registerReportFonts, called once by the API route before renderToBuffer).

// // Both families now ship as variable-axis-only in the Google Fonts repo (no
// // static per-weight cuts), so @react-pdf/renderer resolves everything
// // against a single instance per family. Headings still read as an elegant
// // serif and the sans body still reads clean; you just don't get distinct
// // registered weights within react-pdf's renderer. If you want true
// // multi-weight rendering later, self-host static TTF cuts (e.g. via
// // fontsource) in /public/fonts and point these src values at your own
// // domain instead of GitHub.
...
//   divider: { borderBottomWidth: 0.75, borderBottomColor: brand.border, marginVertical: 14 },
//   footer: {
//     position: "absolute",
//     bottom: 26,
//     left: 52,
...
// function bandColorHex(band: Band) {
//   return bandCopy[band].color;
// }
...
// // ── Domain bar (simple SVG rect, avoids needing a chart library) ──────────

// function DomainBar({ label, percent, color }: { label: string; percent: number; color: string }) {
//   const barWidth = 340;
//   return (
//     <View style={{ marginBottom: 10 }}>
//       <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 3 }}>
//         <Text style={{ fontSize: 9.5, color: brand.ink }}>{label}</Text>
//         <Text style={{ fontSize: 8.5, color, fontWeight: 600 }}>{percent}%</Text>
//       </View>
//       <Svg width={barWidth} height={7}>
//         <Rect x={0} y={0} width={barWidth} height={7} rx={3.5} fill={brand.border} />
//         <Rect x={0} y={0} width={(barWidth * percent) / 100} height={7} rx={3.5} fill={color} />
//       </Svg>
//     </View>
//   );
// }

// // ── Radar-style executive function shape (simple polygon, no chart lib) ───

// function ExecutiveFunctionShape({ result }: { result: AssessmentResult }) {
//   const domains: Domain[] = ["working_memory", "executive_function", "organisation", "emotional_regulation"];
//   const size = 180;
//   const center = size / 2;
//   const maxR = 70;
//   const points = domains.map((d, i) => {
//     const angle = (Math.PI * 2 * i) / domains.length - Math.PI / 2;
//     const dr = result.domainResults.find((r) => r.domain === d);
//     const r = ((dr?.percent ?? 0) / 100) * maxR;
...
//         {axisLines.map((p, i) => (
//           <Path key={i} d={`M${center},${center} L${p[0].toFixed(1)},${p[1].toFixed(1)}`} stroke={brand.border} strokeWidth={0.75} />
//         ))}
//         <Path d={pathD} fill={brand.emeraldLight} stroke={brand.emerald} strokeWidth={1.5} />
//       </Svg>
//     </View>
//   );
// }
...
//         <View style={[styles.card, { marginTop: 16, backgroundColor: brand.emeraldLight, borderColor: brand.emerald }]}>
//           <Text style={{ fontSize: 9.5, color: brand.emerald, lineHeight: 1.6 }}>
//             This is an educational assessment, not a diagnosis. Your responses suggest patterns worth exploring
//             further; only a qualified healthcare professional can determine whether these relate to ADHD or
//             another explanation.
//           </Text>
//         </View>

//         <View style={{ marginTop: 20 }}>
//           <Text style={styles.h2}>Executive function snapshot</Text>
//           <ExecutiveFunctionShape result={result} />
//         </View>

//         <ReportFooter confidentialFor={name} />
//       </Page>

//       {/* Clinical interpretation, clinician-voice prose per domain */}
//       <Page size="A4" style={styles.page}>
...
//             <Text style={{ fontSize: 10.5, fontWeight: 600, color: brand.emerald, marginBottom: 4 }}>{d.label}</Text>
...
//       {/* Domain breakdown */}
//       <Page size="A4" style={styles.page}>
//         <Text style={styles.eyebrow}>Domain by domain</Text>
//         <Text style={styles.h1}>Symptom domain overview</Text>
//         <Text style={[styles.body, { marginBottom: 16 }]}>
//           Each domain below reflects your response pattern in that area, shown as a percentage of the maximum
//           possible score. A higher percentage indicates responses suggesting more frequent difficulty.
//         </Text>

//         <View style={styles.card}>
//           {result.domainResults.map((d) => (
//             <DomainBar key={d.domain} label={d.label} percent={d.percent} color={bandColorHex(d.band)} />
//           ))}
//         </View>

//         <View style={{ flexDirection: "row", gap: 12, marginTop: 8 }}>
//           <View style={[styles.card, { flex: 1 }]}>
//             <Text style={styles.eyebrow}>Potential areas of strength</Text>
//             {result.strengths.map((s) => (
//               <Text key={s.domain} style={[styles.body, { marginBottom: 4 }]}>{"\u2022"} {s.label}</Text>
//             ))}
//           </View>
//           <View style={[styles.card, { flex: 1 }]}>
//             <Text style={styles.eyebrow}>Potential areas of challenge</Text>
//             {result.challenges.map((c) => (
//               <Text key={c.domain} style={[styles.body, { marginBottom: 4 }]}>{"\u2022"} {c.label}</Text>
//             ))}
//           </View>
//         </View>
...
//       {/* Personalised guidance */}
//       <Page size="A4" style={styles.page}>
//         <Text style={styles.eyebrow}>Personalised guidance</Text>
//         <Text style={styles.h1}>Practical strategies for your profile</Text>
//         <Text style={[styles.body, { marginBottom: 14 }]}>
//           These strategies are matched to the domains where your responses indicated the most difficulty.
//           Start with one, rather than all of them at once.
//         </Text>
...
//               <Text style={{ fontSize: 11, fontWeight: 600, color: brand.emerald, marginBottom: 6 }}>{label}</Text>
//               {domainStrategies[domain].map((s, i) => (
//                 <Text key={i} style={[styles.body, { marginBottom: 5 }]}>{"\u2022"} {s}</Text>
//               ))}
//             </View>
//           );
//         })}
...
//         <Text style={styles.h1}>Doctor discussion summary</Text>
...
//           {result.domainResults.map((d) => (
//             <View key={d.domain} style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
//               <Text style={{ fontSize: 9.5, color: brand.ink }}>{d.label}</Text>
//               <Text style={{ fontSize: 9.5, color: bandColorHex(d.band) }}>{bandCopy[d.band].label} ({d.percent}%)</Text>
//             </View>
//           ))}
//         </View>

//         <View style={[styles.card, { marginTop: 8 }]}>
//           <Text style={{ fontSize: 9.5, fontWeight: 600, marginBottom: 6 }}>Reason for the visit</Text>
//           <Text style={styles.body}>
//             I completed a self-guided educational ADHD screening through Mentel and would like to discuss
//             whether these patterns warrant further evaluation.
//           </Text>
//         </View>

//         <Text style={[styles.small, { marginTop: 16 }]}>{reportDisclaimer}</Text>
...
//           Mentel. This report was generated on {completionDate} and reflects your responses at
