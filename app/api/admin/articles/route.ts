...
// GET: list all articles (any status) for the admin CMS.
...
async function POST_HANDLER(req: Request) {
  ...
  try {
    ...
    if (!title || title.length < 3) {
      return NextResponse.json({ success: false, errors: { title: "Title is required." } }, { status: 400 });
    }
    if (!content || content.length < 20) {
      return NextResponse.json({ success: false, errors: { content: "Content is required." } }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json(
        { success: false, errors: { slug: "An article with this slug already exists." } },
        { status: 400 }
      );
    }
    ...
    const article = await db.article.create({
      data: {
        ...
        readMin: Number.isFinite(body.readMin) ? Math.max(1, Math.round(body.readMin)) : Math.max(1, Math.round(content.split(/\s+/).length / 200)),
