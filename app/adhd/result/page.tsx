...
function AdhdResultContent() {
    ...
    // itself, easy to miss if they'd scrolled away from it or the card was
    // off-screen (the sidebar on desktop, for instance).
    function handlePaymentSuccess(paidTxRef: string) {
        setUnlocked(true);
        setShowSuccessModal(true);
        // Bug fix: previously this function took no argument and never
        // captured the tx_ref Flutterwave's in-page `callback` actually
        // provides (see FlutterwaveCheckout.tsx's `onSuccess(init.txRef)`).
        // For anyone whose payment completed entirely in-modal, no page
        // redirect at all (the common case for card payments), the URL
        // never had a `tx_ref` query param to begin with, so
        // `effectiveTxRef` stayed null and the download link/PDF route
        // call ended up pointing at `tx_ref=` (empty). Capturing it here
        // fixes that regardless of which completion path fired.
        setRecoveredTxRef((prev) => prev ?? paidTxRef);
        // Purchase, the highest-value event in this funnel. Generic
        // contentName again, same reasoning as ViewContent/Lead above.
        fireConversion("Purchase", {
            contentName: "self_assessment",
            value: PRIMARY_PLAN.amountUSD,
            currency: "USD",
            dedupeKey: paidTxRef,
            eventId: paidTxRef,
        });
            ...
            {!unlocked && showMobileBar && !mobileBarDismissed && (
                ...
                >
                    ...
                    <button
                        onClick={() => setMobileBarDismissed(true)}
                        aria-label="Dismiss"
                        className="flex-shrink-0 cursor-pointer w-8 h-8 flex items-center justify-center rounded-full text-[#4a5a52] hover:bg-[#f5f5f2]"
                    >
                        <XIcon size={16} />
                    </button>
                </div>
            ...
            {unlocked && stored && !successBarDismissed && (
                ...
                >
                    ...
                    <button
                        onClick={() => setSuccessBarDismissed(true)}
                        aria-label="Dismiss"
                        className="flex-shrink-0 cursor-pointer w-7 h-7 flex items-center justify-center rounded-full text-white/70 hover:bg-white/10"
                    >
                        <XIcon size={15} />
                    </button>
                </div>
        ...
        >
            <div className="relative bg-white rounded-[28px] max-w-[420px] w-full p-8 text-center shadow-[0_30px_90px_rgba(14,92,61,0.3)] fade-up-modal">
                <button
                    onClick={onClose}
                    aria-label="Close"
                    className="absolute cursor-pointer top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-[#4a5a52] hover:bg-[#f5f5f2]"
                >
                    <XIcon size={16} />
                </button>
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#2d7a5a] to-[#1e6b6b] inline-flex items-center justify-center mb-5">
                ...
                <button
                    ...
                    className="text-[13px] text-[#4a5a52] hover:text-[#1c2820] py-1 cursor-pointer"
                >
                    View my results page
                </button>
                <p className="text-[11px] text-[#4a5a52] mt-5 leading-[1.5]">
    ...
    return (
        <div className="bg-white rounded-2xl border border-[#e4e9e5] overflow-hidden">
            <button
                ...
                aria-expanded={open}
                aria-controls={panelId}
                className="w-full flex cursor-pointer items-center justify-between gap-3 px-6 py-5 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2d7a5a] focus-visible:outline-offset-[-2px]"
            >
                <span>
                    <span className="block text-[14px] font-semibold text-[#1c2820]">View your full domain-by-domain breakdown</span>
                    <span className="block text-[12.5px] text-[#4a5a52] mt-0.5">All 8 areas, explained one at a time</span>
        ...
        <div className="bg-white rounded-2xl border border-[#e4e9e5] overflow-hidden">
            <h3>
                <button
                    ...
                    aria-expanded={open}
                    aria-controls={panelId}
                    className="w-full flex cursor-pointer items-center justify-between gap-3 px-6 py-4 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2d7a5a] focus-visible:outline-offset-[-2px]"
                >
                    <span className="text-[13px] font-semibold tracking-[0.03em] text-[#2d7a5a]">{label}</span>
                    <ChevronDown size={17} aria-hidden="true" className="text-[#4a5a52] flex-shrink-0 transition-transform duration-200" style={{ transform: open ? "rotate(180deg)" : "none" }} />
                </button>
        ...
        <div className="bg-white rounded-3xl border border-[#e4e9e5] overflow-hidden">
            <h2>
                <button
                    ...
                    aria-expanded={open}
                    aria-controls={panelId}
                    className="w-full flex cursor-pointer items-center justify-between gap-3 px-7 py-6 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2d7a5a] focus-visible:outline-offset-[-2px]"
                >
                    <span className="font-['Cormorant_Garamond',Georgia,serif] text-[22px] font-light text-[#1c2820]">
                        What this means
                    </span>
        ...
        <div className="bg-white rounded-xl border border-[#e4e9e5] overflow-hidden">
            <h3>
                <button
                    ...
                    aria-expanded={open}
                    aria-controls={panelId}
                    className="w-full flex cursor-pointer items-center justify-between gap-3 px-5 py-4 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2d7a5a] focus-visible:outline-offset-[-2px]"
                >
                    <span className="text-[13.5px] font-medium text-[#1c2820]">{q}</span>
                    <ChevronDown size={16} aria-hidden="true" className="text-[#4a5a52] flex-shrink-0 transition-transform duration-200" style={{ transform: open ? "rotate(180deg)" : "none" }} />
                </button>
...
function PricingCard({ stored, verifying, onSuccess, compact = false }: { stored: StoredResult; verifying: boolean; onSuccess: (txRef: string) => void; compact?: boolean }) {
    return (
        <div className={`bg-white rounded-[28px] border border-[#e4e9e5] shadow-[0_16px_60px_rgba(28,40,36,0.1)] overflow-hidden ${compact ? "" : "max-w-[440px] mx-auto text-center"}`}>
            <div className="h-[3px] bg-gradient-to-r from-[#2d7a5a] via-[#1e6b6b] to-[#5da885]" />
            <div className={compact ? "p-6 text-center" : "p-9"}>
                ...
                <FlutterwaveCheckout
                    ...
                    customer={{ name: stored.name, email: stored.email, phone: stored.phone }}
                    onOpen={(txRef) =>
                        fireConversion("InitiateCheckout", {
                            contentName: "self_assessment",
                            value: PRIMARY_PLAN.amountUSD,
                            currency: "USD",
                            dedupeKey: txRef,
