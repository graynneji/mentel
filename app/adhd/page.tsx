...
// import { SocialProofBar, AsSeenOn, TrustSection, Testimonials } from "@/components/adhd/TrustAndProof";
// import { socialProofStats, testimonials, pressOutlets, clinicallyReviewed } from "@/lib/adhd/social-proof-config";
...
//     // Scroll question card into view smoothly on question change(auto - scroll UX)
//     useEffect(() => {
//         if (step !== "quiz") return;
...
//             const payload = { name: name.trim(), email: email.trim(), phone: phone.trim(), answers, result, completedAt: Date.now() };
//             try {
//                 window.sessionStorage.setItem("mentel_adhd_result", JSON.stringify(payload));
//             } catch { /* sessionStorage unavailable — result page will fall back to answers only */ }
//             // Fire-and-forget lead capture so the funnel doesn't stall on network latency
//             fetch("/api/adhd/lead", {
//                 method: "POST",
...
//         return (
//             <PageWrapper>
//                 <section className="pt-[112px] pb-24 px-6 min-h-screen">
//                     <div className="max-w-[620px] mx-auto text-center fade-up">
//                         <h1 className="font-['Cormorant_Garamond',Georgia,serif] text-[clamp(34px,7.5vw,54px)] font-light tracking-[-0.025em] text-[#1c2820] leading-[1.12] mb-5">
//                             Understand your attention.
//                             <br />Discover your patterns.
//                         </h1>
//                         <p className="text-[16.5px] font-light text-[#5a6b5e] leading-[1.75] max-w-[500px] mx-auto mb-3">
...
//                         </p>
//                         <p className="text-[14px] font-light text-[#6a7a6e] leading-[1.7] max-w-[440px] mx-auto mb-8">
...
//                             {["3\u20134 minutes", "Private", "Evidence-informed", "Not a diagnosis", "No signup required"].map((tag) => (
//                                 <span key={tag} className="text-[12px] text-[#4a6a56] font-medium flex items-center gap-1.5">
...
//                                     <button onClick={handleStartFresh} className="text-[13px] text-[#6a7a6e] px-4 py-2 rounded-full border border-[#d8dbd5] bg-white hover:bg-[#f5f5f2] transition-colors">Start fresh</button>
...
//                                 </button>
//                                 <p className="text-[12px] text-[#6a7a6e] mt-3">No credit card &middot; 4 minutes &middot; {TOTAL_QUESTIONS} questions</p>
//                             </>
//                         )}
                ...
                <section className="pt-[84px] sm:pt-[112px] pb-24 px-6 min-h-screen">
                    <div className="max-w-[620px] mx-auto text-center fade-up">
                        ...
                        {resumeAvailable ? (
                            ...
                            <>
                                <button onClick={() => setStep("quiz")}
                                    className="cta-btn w-full sm:w-auto py-[18px] px-11 bg-gradient-to-br cursor-pointer from-[#2d7a5a] to-[#1e6b6b] text-white border-0 rounded-full text-[15.5px] font-medium flex items-center justify-center gap-2 mx-auto shadow-[0_6px_26px_rgba(30,107,107,0.32)]">
                                    Start Free Assessment
                                    <ArrowRight size={16} strokeWidth={2} />
                                </button>
                        ...
                        {!resumeAvailable && (
                            <button onClick={() => setStep("quiz")}
                                className="cta-btn w-full sm:w-auto py-[17px] px-10 bg-gradient-to-br cursor-pointer from-[#2d7a5a] to-[#1e6b6b] text-white border-0 rounded-full text-[15px] font-medium flex items-center justify-center gap-2 mx-auto shadow-[0_4px_22px_rgba(30,107,107,0.3)] mt-4">
                                Start Free Assessment
                                <ArrowRight size={16} strokeWidth={2} />
                            </button>
                        )}
