...
//                                 className="w-full py-[16px] px-7 rounded-full text-white text-[15px] font-medium flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0 mb-3 cursor-pointer"
...
//                                             className="w-full py-[14px] px-6 rounded-full text-white text-[14px] font-medium flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5 cursor-pointer"
//                                             style={{ background: `linear-gradient(135deg, ${band.accentColor}, ${band.accentColor}bb)`, boxShadow: `0 4px 18px ${band.accentColor}44` }}>
...
export default function BookCallContent() {
    ...
    const handleBook = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok || !json.success) { setBookingError(json.error ?? "Something went wrong. Please try again."); return; }
            markBooked();
            fireConversion("Schedule", { contentName: "book_call" });
            setStep("success");
        } catch { setBookingError("Network error. Please try again."); }
        finally { setSubmitting(false); }
    }, [selectedSlot, name, email, notes]);
