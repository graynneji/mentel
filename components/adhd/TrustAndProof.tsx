...
import { ReactNode } from "react";
import { Shield, ShieldCheck, Lock, Users, Stethoscope, FileCheck } from "lucide-react";
...
// ── Clinical trust bar ───────────────────────────────────────────────────
// A more visually prominent icon strip than TrustSection below, for
// wherever you want a stronger "medical presence" on the page. Deliberately
// limited to icon+claim pairs that are true without needing third-party
// verification: encryption and privacy practices you control directly, and
// "reviewed by a licensed clinician" gated behind the same
// `clinicallyReviewed` flag as TrustSection, off until that review has
// actually happened.
//
// What's NOT here: a "HIPAA Compliant" badge. That's a specific,
// checkable claim about signed Business Associate Agreements with every
// vendor touching this data, encryption at rest and in transit, access
// controls, audit logging, and a documented risk assessment, not something
// that follows from having a Clinical Director, and not something I can
// verify is true of your actual infrastructure. It's real regulatory
// exposure (HHS OCR and the FTC both enforce against this) if displayed
// without an actual compliance program behind it. If you've done that
// assessment and have BAAs in place, add it with your compliance counsel's
// sign-off, this component is intentionally not the place a claim like
// that gets added silently.
export function ClinicalTrustBar({ clinicallyReviewed = false, hipaaAligned = false }: { clinicallyReviewed?: boolean; hipaaAligned?: boolean }) {
    // hipaaAligned: off by default on purpose. There's no official HIPAA
    // "certified" seal, no government body issues one, so this
    // deliberately says "HIPAA-aligned practices" rather than "HIPAA
    // Certified" or "HIPAA Compliant", the former is defensible as a
    // description of your actual practices, the latter reads as a formal
    // certification that doesn't exist. Flip this to true once you're
    // ready, and it's worth having your own copy for the underlying claim
    // (a Privacy & Security page describing what's actually in place)
    // rather than resting the whole claim on an icon.
    const items: { icon: ReactNode; text: string }[] = [
        { icon: <Lock size={20} />, text: "256-bit encryption" },
        { icon: <ShieldCheck size={20} />, text: "Private by design" },
        ...(clinicallyReviewed ? [{ icon: <Stethoscope size={20} />, text: "Clinician-reviewed content" }] : []),
        ...(hipaaAligned ? [{ icon: <FileCheck size={20} />, text: "HIPAA-aligned practices" }] : []),
        { icon: <Users size={20} />, text: "Built with licensed clinicians" },
    ];
    return (
        <div className="flex flex-wrap items-center justify-center gap-x-9 gap-y-4 py-6">
            {items.map((item) => (
                <div key={item.text} className="flex flex-col items-center gap-2 w-[92px] text-center">
                    <div className="w-11 h-11 rounded-full bg-[#f2f7f3] border border-[#d5e5da] flex items-center justify-center text-[#0E5C3D]">
                        {item.icon}
                    </div>
                    <span className="text-[11px] font-medium text-[#4a5a52] leading-tight">{item.text}</span>
                </div>
            ))}
        </div>
    );
