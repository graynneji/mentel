...
import Link from "next/link";
import { ArrowRight, Leaf, ClipboardCheck } from "lucide-react";
import { services } from "@/lib/services-data";

const SITE_URL = "https://trymentel.com";

export const metadata: Metadata = {
    title: "Online Therapy Services in Nigeria | Mentel LTD",
    description:
        "Licensed therapists in Nigeria offering online therapy for anxiety, depression, couples, trauma, burnout and self-esteem. Confidential sessions. Book a free consultation.",
    keywords: [
    ...
    openGraph: {
        title: "Online Therapy Services in Nigeria | Mentel LTD",
        description:
            "Licensed therapists in Nigeria offering online therapy for anxiety, depression, couples, trauma, burnout and self-esteem.",
    ...
    return (
        <div className="relative">
            ...
            <div className="relative z-10 max-w-5xl mx-auto px-6 pt-28 pb-20">
                ...
                {/* SEO intro paragraph */}
                <p className="text-sm leading-relaxed mb-14" style={{ color: "var(--text-muted)" }}>
                    Mentel connects you with licensed therapists across Nigeria for confidential, online
                    mental health support. Whether you are managing anxiety, working through depression,
                    rebuilding a relationship, processing trauma, recovering from burnout, or building
                    self-esteem, our therapists use evidence-based approaches tailored to your situation.
                    Explore each service below to learn more about how we can help.
                </p>

                {/* Services Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-16">
                    {services.map(({ slug, shortTitle, shortDesc, icon: Icon, tags }) => (
                        <Link
                            href={`/services/${slug}`}
                            key={slug}
                            className="p-6 rounded-2xl border group transition-all duration-200 hover:-translate-y-1 hover:shadow-md"
                        ...
                        >
                            ...
                            <h2 className="font-cormorant text-xl font-semibold mb-2" style={{ color: "var(--deep)" }}>
                                {shortTitle}
                            </h2>
                            <p className="text-sm leading-relaxed mb-4" style={{ color: "var(--text-muted)" }}>{shortDesc}</p>
                            ...
                            <div
                                className="inline-flex items-center gap-1.5 text-xs font-medium transition-transform group-hover:translate-x-0.5"
                    ...
                    {/* Free Assessment card stays as-is, links to the assessment flow */}
                    <Link
                        href="/assessment"
                        className="p-6 rounded-2xl border group transition-all duration-200 hover:-translate-y-1 hover:shadow-md"
                        style={{ background: "white", borderColor: "var(--border)" }}
                    >
                        <div
                            ...
                            style={{ background: "rgba(123,169,139,0.12)" }}
                        >
                            <ClipboardCheck size={17} style={{ color: "var(--sage-dark)" }} />
                        </div>
                        <h2 className="font-cormorant text-xl font-semibold mb-2" style={{ color: "var(--deep)" }}>
                            Free Assessment
                        </h2>
                        <p className="text-sm leading-relaxed mb-4" style={{ color: "var(--text-muted)" }}>
                            Not sure where to start? Take our 2-minute mental health check.
                        </p>
                        <div className="flex flex-wrap gap-2 mb-4">
                            {["Free Mental Health Assessment", "Depression Assessment", "Check Anxiety"].map((tag) => (
                                <span
                                    key={tag}
                                    className="text-xs px-2.5 py-1 rounded-full"
                                    style={{ background: "var(--mist)", color: "var(--sage-dark)" }}
